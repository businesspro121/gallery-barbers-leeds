...
/* ── Gallery Barbers brand tokens ───────────────────────────────────────────
   These mirror the CSS custom properties in assets/css/styles.css so the cloth
   sits in the same palette as the rest of the site. Change them here and the
   woven texture, the lighting and the overlay all follow.                    */
export const WOVEN_CLOTH_BRAND = {
  ink: "#0B0B0C",        // --ink-900, page ground
  inkRaised: "#131315",  // --ink-800
  ivory: "#F4F0E8",      // --ivory, 16.8:1 on ink
  ivoryDim: "#DBD5C9",
  stone: "#A9A9B0",      // --stone-300
  gold: "#C6A15B",       // --gold, 7.9:1 on ink
  goldBright: "#E0C088",
  goldDeep: "#7A6030",   // --gold-ink
} as const;

/* The wordmark woven into the cloth. */
...
const WOVEN_CLOTH_TITLE = "Gallery Barbers woven cape";
const WOVEN_CLOTH_BACKGROUND = WOVEN_CLOTH_BRAND.ink;
...
// Self-contained HTML/Three.js document: a barber's cape pinned along the top,
// moving in the draught, with the shop's name woven into the cloth.
const galleryBarbersClothSource = `<!DOCTYPE html>
<html lang="en-GB">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
    <title>Gallery Barbers · Unisex Barber Shop, Leeds Kirkgate Market</title>
...
<body class="text-[#F4F0E8] overflow-hidden antialiased selection:bg-[#C6A15B] selection:text-[#0B0B0C] font-serif" style="background: radial-gradient(120% 100% at 50% 30%, #1C1C20 0%, #131315 55%, #0B0B0C 100%); height: 100dvh; width: 100vw;">

    <!-- Stage / Background -->
    <div class="fixed inset-0 overflow-hidden z-0">
        <canvas id="cloth" class="absolute inset-0 w-full h-full block pointer-events-none"></canvas>
        <div class="absolute inset-0 pointer-events-none" style="background: radial-gradient(90% 80% at 50% 46%, transparent 55%, rgba(11,11,12,.78) 100%);"></div>
    </div>
    ...
    <div class="absolute inset-0 z-10 flex flex-col p-6 md:p-12 pointer-events-none font-sans">
        ...
        <header class="flex justify-between items-center w-full pointer-events-auto">
            <div class="overflow-hidden">
                <div class="reveal-item opacity-0 translate-y-8 text-xs tracking-[0.28em] uppercase text-[#C6A15B] font-medium">
                    Stall 10–11 · Leeds Kirkgate Market
                </div>
            </div>
            <nav class="hidden md:flex gap-8">
                <div class="overflow-hidden">
                    <a href="#services" class="reveal-item block opacity-0 translate-y-8 text-xs tracking-widest uppercase text-[#F4F0E8]/80 hover:text-white transition-colors duration-300">Services</a>
                </div>
                <div class="overflow-hidden">
                    <a href="#gallery" class="reveal-item block opacity-0 translate-y-8 text-xs tracking-widest uppercase text-[#F4F0E8]/80 hover:text-white transition-colors duration-300">Gallery</a>
                </div>
                <div class="overflow-hidden">
                    <a href="#visit" class="reveal-item block opacity-0 translate-y-8 text-xs tracking-widest uppercase text-[#F4F0E8]/80 hover:text-white transition-colors duration-300">Visit</a>
                </div>
            </nav>
        </header>
        ...
        <div class="mt-auto flex flex-col sm:flex-row justify-between items-start sm:items-end gap-8 pointer-events-auto w-full">
            ...
            <div class="max-w-md space-y-6">
                <div>
                    <p class="word-reveal text-sm md:text-base leading-relaxed text-[#F4F0E8]/90">
                        <strong class="text-white font-semibold block mb-1 text-base md:text-lg tracking-tight font-serif">Precision cuts. Leeds style.</strong>
                        Sharp cuts, clean fades and welcoming service in the heart of Leeds Kirkgate Market. Cutting for men, women and children.
                    </p>
                </div>

                <div class="overflow-hidden pt-2">
                    <div class="reveal-item opacity-0 translate-y-8 inline-block rounded-sm p-[1px] bg-gradient-to-b from-white/30 via-[#E0C088]/30 to-[#7A6030]/10 shadow-[0_10px_30px_rgba(122,96,48,0.4)] transition-transform duration-300 hover:-translate-y-0.5 group">
                        <a href="#book" class="flex items-center gap-3 bg-[#C6A15B] group-hover:bg-[#E0C088] text-[#0B0B0C] px-6 py-3 rounded-[1px] text-sm font-semibold tracking-wide transition-colors relative overflow-hidden">
                            <span class="relative z-10">Book your appointment</span>
            ...
            <div class="text-left sm:text-right mt-8 sm:mt-0">
                <div class="word-reveal text-xs tracking-[0.24em] uppercase text-[#C6A15B] leading-[1.9] font-medium">
                    Rated 4.9 on Google<br>
                    Mon–Sat · 9:00–17:30<br>
                    Walk-ins welcome
                </div>
            </div>
    ...
    <script>
        ...
        (() => {
            ...
            // Generate the woven texture: charcoal cloth, antique-gold hem.
            function makeClothTexture() {
                ...
                // Antique-gold hem
                x.strokeStyle = '#C6A15B';
                x.lineWidth = 9;
                x.strokeRect(46, 46, W-92, H-92);
                x.lineWidth = 3;
                x.strokeStyle = '#7A6030';
                ...
                // Weave overlay: warp and weft threads
                x.globalAlpha = 1;
                ...
                for(let xx=0; xx<W; xx+=3){
                    x.strokeStyle = 'rgba(255,246,230,0.045)';
                    x.lineWidth = 1;
            ...
            // Lighting: warm key, antique-gold rim
...
/**
 * Stamps WOVEN_CLOTH_WORDMARK into the canvas texture. Editing the exported
 * constant is all it takes to reword the cloth — the strings below are the
 * anchors in the embedded document, not the copy itself.
 */
function wovenClothLabelSource(source: string) {
  const w = WOVEN_CLOTH_WORDMARK;
  return (
    [
      ["x.fillText('G B', W/2, 176);", `x.fillText('${w.monogram}', W/2, 176);`],
      [
        "x.fillText('· LEEDS ·', W/2, 232);",
        `x.fillText('${w.eyebrow}', W/2, 232);`,
      ],
      ["x.fillText('GALLERY', W/2, 372);", `x.fillText('${w.lineOne}', W/2, 372);`],
      ["x.fillText('BARBERS', W/2, 488);", `x.fillText('${w.lineTwo}', W/2, 488);`],
      [
        "x.fillText('K I R K G A T E   M A R K E T', W/2, 590);",
        `x.fillText('${w.footerOne}', W/2, 590);`,
      ],
      [
        "x.fillText('S T A L L   1 0 – 1 1', W/2, 632);",
        `x.fillText('${w.footerTwo}', W/2, 632);`,
      ],
    ] as const
  ).reduce(
...
function buildFocusedDocument(mode: EffectMode) {
  ...
  const background =
    mode === "light" ? WOVEN_CLOTH_BRAND.inkRaised : WOVEN_CLOTH_BACKGROUND;
...
(function () {
  ...
  const presentedSource = wovenClothLabelSource(galleryBarbersClothSource);
